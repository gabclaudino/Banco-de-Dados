...
#define MAX_OPS 10000    // máximo de operações total
#define MAX_TX 100       // máximo transações por escalonamento
#define MAX_ATTR 32      // tamanho máximo do atributo
...
// Representa um escalonamento (lista de operações)
typedef struct {
    Op ops[MAX_OPS];
    int n_ops;
} Schedule;
...
int adj[MAX_TX][MAX_TX];   // matriz de adjacência
int n_tx;                  // número de transações distintas
int tx_ids[MAX_TX];        // lista de tids no escalonamento
int cor[MAX_TX];           // para DFS (0=branco,1=cinza,2=preto)

// Estrutura para capturar informações de leituras
typedef struct {
    int tid;               // transação que leu
    char attr[MAX_ATTR];   // atributo lido
    int rank;              // enésima leitura desse atributo por essa transação
    int writer;            // tid do escritor de quem leu (-1 se inicial)
} ReadInfo;

// Gera próxima permutação lexicográfica (retorna 1 se há próxima)
int next_permutation(int *a, int n) {
    ...
    int l = i+1, r = n-1;
    while (l < r) { tmp = a[l]; a[l++] = a[r]; a[r--] = tmp; }
    return 1;
}


// Função: encontra índice de um tid na lista tx_ids
int index_of(int tid) {
    for (int i = 0; i < n_tx; i++) {
        if (tx_ids[i] == tid) return i;
    }
    return -1;
}

// Coleta tids de um escalonamento, em ordem de aparição
...
int dfs_ciclo(int u) {
    ...
    for (int v = 0; v < n_tx; v++) {
        if (adj[u][v]) {
            if (cor[v] == 1) return 1;
            if (cor[v] == 0 && dfs_ciclo(v)) return 1;
        }
    }
    cor[u] = 2;
    return 0;
...
int teste_seriabilidade(Schedule *sch) {
    ...
    for (int i = 0; i < n_tx; i++)
        for (int j = 0; j < n_tx; j++)
            adj[i][j] = 0;
    // cria arestas para cada par de operações conflitantes
    for (int i = 0; i < sch->n_ops; i++) {
        Op *ei = &sch->ops[i];
    ...
    // detecta ciclo
    for (int i = 0; i < n_tx; i++) cor[i] = 0;
    for (int i = 0; i < n_tx; i++) {
        if (cor[i] == 0 && dfs_ciclo(i)) return 0; // ciclo -> não serial
    }
    return 1; // sem ciclos -> serial
}

// Gera escalonamento serial a partir de permutação de tids
void gerar_serial(Schedule *ser, Schedule *orig, int perm[]) {
    ser->n_ops = 0;
    for (int k = 0; k < n_tx; k++) {
        int tid = tx_ids[perm[k]];
        for (int i = 0; i < orig->n_ops; i++) {
            if (orig->ops[i].tid == tid) ser->ops[ser->n_ops++] = orig->ops[i];
        }
    }
}

// Teste de equivalência de visão
int teste_visao(Schedule *orig) {
    ...
    for (int i = 0; i < m; i++) {
        if (orig->ops[i].op == 'R') {
            ...
            // identifica writer que forneceu valor (último W antes)
            int writer = -1;
            for (int k = i-1; k >=0; k--) {
    ...
    // identifica último escritor de cada atributo em orig
    int last_writer_attr[256]; int has_last[256] = {0};
    for (int i = 0; i < m; i++) {
        if (orig->ops[i].op=='W') {
            unsigned char h = 0;
            for (char *p = orig->ops[i].attr; *p; p++) h += *p;
            last_writer_attr[h] = orig->ops[i].tid;
            has_last[h] = 1;
        }
    }
    // gera permutações possíveis e testa
    int perm[MAX_TX]; for (int i=0;i<n_tx;i++) perm[i]=i;
    Schedule ser;
    do {
        gerar_serial(&ser, orig, perm);
        // captura leituras em ser
        ReadInfo ser_reads[MAX_OPS]; int n_ser_reads = 0;
        ...
        for (int r = 0; r < n_reads && ok; r++) {
            ...
            for (int s = 0; s < n_ser_reads; s++) {
                if (orig_reads[r].tid==ser_reads[s].tid &&
                    ...
                    if (orig_reads[r].writer != ser_reads[s].writer) ok=0;
                    found = 1; break;
                }
            }
            if (!found) ok=0;
        }
        // compara último escritor
        for (int h=0;h<256 && ok;h++) {
            if (has_last[h]) {
                // em ser: busca último W para esse h
                int lw = -1; int present=0;
                for (int i=0;i<ser.n_ops;i++) {
                    if (ser.ops[i].op=='W') {
                        unsigned char hh=0;
                        for (char *p=ser.ops[i].attr;*p;p++) hh+=*p;
                        if (hh==h) { lw=ser.ops[i].tid; present=1; }
                    }
                }
                if (!present || lw!= last_writer_attr[h]) ok=0;
            }
        }
        if (ok) return 1;
...
// Verifica se tid já está na lista
typedef struct { int tid; } Active;
int presente(int *arr, int n, int tid) {
    for (int i = 0; i < n; i++) if (arr[i] == tid) return 1;
    return 0;
}
